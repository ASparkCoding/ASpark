...
export function HeroWave({
  ...
  extendLeftPx = 320,
  title = "描述你的想法\n获得一个可运行的应用",
  subtitle = "ASpark 通过自然语言生成全栈 Web 应用 — 开源、多模型 AI、零厂商锁定。",
  ...
  useEffect(() => {
    ...
    function createInstancedMaterial() {
      // ASpark brand: warm orange/vermilion instead of blue
      const baseCol = new THREE.Color("#E04E2A");
      const emisCol = new THREE.Color("#C4421F");
    ...
    function buildBars(w: number) {
      if (instancedBars) {
        waveScene.remove(instancedBars);
        instancedBars.geometry.dispose();
        instancedBars = null;
      }
      // Cover full viewport width plus extension on both sides
      const totalW = w + EXTEND_LEFT_PX * 2;
      const count = Math.min(
        MAX_BARS,
        Math.ceil(totalW / (FIXED_BAR_WIDTH + FIXED_BAR_GAP))
      );
      ...
      const mesh = new THREE.InstancedMesh(geo, barMaterial, count);
      mesh.frustumCulled = false;

      const xArr = new Float32Array(count);
      const normArr = new Float32Array(count);
      const groupArr = new Float32Array(count);
      const glowArr = new Float32Array(count);
      barCenters = new Float32Array(count);

      // Start from left edge of camera view minus extension
      const startX = -(w / 2) - EXTEND_LEFT_PX;
      for (let i = 0; i < count; i++) {
        const x = startX + i * (FIXED_BAR_WIDTH + FIXED_BAR_GAP);
        xArr[i] = x;
        barCenters[i] = x;
        normArr[i] = i / (count - 1 || 1);
        groupArr[i] = i % 2 === 0 ? 0 : 1;
        glowArr[i] = 0;
      ...
      geo.setAttribute("aXPos", new THREE.InstancedBufferAttribute(xArr, 1));
      geo.setAttribute(
        "aPosNorm",
        new THREE.InstancedBufferAttribute(normArr, 1)
      );
      geo.setAttribute(
        "aGroup",
        new THREE.InstancedBufferAttribute(groupArr, 1)
      );
    ...
    function accumulateGlow(dt: number) {
      ...
      const geo = instancedBars.geometry;
      const glowAttr = geo.getAttribute("aGlow") as THREE.InstancedBufferAttribute;
      const arr = glowAttr.array as Float32Array;
      const mousePx = proxyMouseX;

      for (let i = 0; i < currentBarCount; i++) {
        const cx = barCenters[i] + cameraWidth * 0.5;
        const dist = Math.abs(mousePx - cx);
        const inRange =
          mouse.active && dist < glowConfig.maxGlowDistance ? 1 : 0;

        if (inRange) {
          const proximity =
            1 -
            Math.pow(
              dist / glowConfig.maxGlowDistance,
              glowConfig.glowFalloff
            );
          arr[i] = Math.min(
            glowDynamics.max,
            arr[i] + proximity * glowDynamics.accumulation * glowDynamics.accumEase * dt
          );
        } else {
          arr[i] = Math.max(0, arr[i] - glowDynamics.decay * dt);
        }
      }
    ...
    function buildScene1Timeline() {
      ...
      for (let i = 0; i < waveKeyframes1.length - 1; i++) {
        const from = waveKeyframes1[i];
        const to = waveKeyframes1[i + 1];
        const dur = to.time - from.time;
        tl.to(wave1, { gain: to.gain, frequency: to.frequency, waveLength: to.waveLength, duration: dur }, from.time);
      }
      for (let i = 0; i < waveKeyframes2.length - 1; i++) {
        const from = waveKeyframes2[i];
        const to = waveKeyframes2[i + 1];
        const dur = to.time - from.time;
        tl.to(wave2, { gain: to.gain, frequency: to.frequency, waveLength: to.waveLength, duration: dur }, from.time);
      }
      return tl;
    }
    ...
    function onResize(w: number, h: number) {
      ...
      if (!waveCameraInitialized) {
        ...
        waveBloomPass = new UnrealBloomPass(
          new THREE.Vector2(w, h), 0.6, 0.4, 0.2
        );
        grainPass = createFilmGrainPass(0.9, 0.3);
        ...
        waveCamera.left = -w / 2;
        waveCamera.right = w / 2;
        waveCamera.top = h / 2;
        waveCamera.bottom = -h / 2;
        waveCamera.updateProjectionMatrix();
        buildBars(w);
      }

      if (barMaterial) {
        (barMaterial.uniforms as any).uHalfW.value = w / 2;
      }
      updateGainMultiplier();

      if (heavyResizeTimer) clearTimeout(heavyResizeTimer);
      heavyResizeTimer = setTimeout(applyHeavyResize, 120);
    ...
    const ticker = () => {
      ...
      const rawSpeed =
        Math.hypot(dx, dy * 0.1) * glowConfig.speedScale;
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-center mb-6">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-brand/30 bg-brand/10 backdrop-blur-sm">
              <span className="w-2 h-2 rounded-full bg-brand animate-pulse" />
              <span className="text-brand text-xs font-medium">
                开源 &middot; AI 驱动的全栈应用生成
              </span>
            </span>
          </div>
          <h1 className="text-white text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight drop-shadow-[0_1px_8px_rgba(224,78,42,0.25)] whitespace-pre-line leading-[1.1]">
            {title}
          </h1>
          <p className="text-gray-300/90 mt-3 sm:mt-5 text-sm sm:text-base max-w-xl mx-auto leading-relaxed">
            {subtitle}
          </p>
          <form
            ...
            <div className="relative w-full sm:w-[720px]">
              <div className="relative rounded-2xl p-[2px] shadow-[0_1px_2px_0_rgba(0,0,0,0.06)] bg-gradient-to-br from-white/10 via-white/5 to-black/20">
                <textarea
                  ...
                  rows={5}
                  className="w-full h-32 sm:h-36 resize-none rounded-2xl bg-[rgba(15,15,20,0.55)] border border-white/10 text-white placeholder:text-white/40 outline-none focus:ring-2 focus:ring-brand/40 focus:border-brand/40 backdrop-blur-md px-4 py-4 pr-16"
                />
              </div>
              <button
                type="submit"
                aria-label={buttonText}
                className="absolute right-3 bottom-3 inline-flex items-center justify-center w-10 h-10 rounded-xl bg-brand text-white hover:bg-brand-light transition-colors"
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mt-5">
            {["仪表盘", "电商平台", "聊天应用", "数据分析", "CRM"].map(
              (tag) => (
                <button
                  key={tag}
                  onClick={() =>
                    setPrompt(`帮我构建一个${tag}应用，使用现代化 UI`)
                  }
                  className="px-3.5 py-1.5 rounded-full border border-white/15 text-xs text-gray-300 hover:border-brand/40 hover:text-white transition-colors backdrop-blur-sm bg-white/5"
                >
                  {tag}
                </button>
              )
